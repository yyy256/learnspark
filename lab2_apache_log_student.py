...
uniqueHosts = hosts.distinct().collect()

uniqueHostCount = uniqueHosts.__len__()
...
dayHostCount = dayGroupedHosts.mapValues(list)

dailyHosts = dayHostCount.map(lambda x: (x[0], len(set(x[1])))).sortByKey().cache() 
...
sortedByDay = groupedByDay.mapValues(list)

avgDailyReqPerHost = sortedByDay.map(lambda x: (x[0], len(x[1])/len(set(x[1])))).sortByKey().cache()
...
badEndpointsSum = badEndpointsCountPairTuple.countByKey().items()

badEndpointsTop20 = sorted(badEndpointsSum, key=lambda x:x[1], reverse = True)[:20]
...
errHostsSum = errHostsCountPairTuple.countByKey().items()

errHostsTop25 = sorted(errHostsSum, key=lambda x:x[1], reverse = True)[:25]
...
errDateSum = errDateCountPairTuple.countByKey()

errDateSorted = sc.parallelize(errDateSum.items()).cache()
...
daysWithErrors404 = map(lambda x: x[0], errDateSorted.collect())
errors404ByDay = map(lambda x: x[1], errDateSorted.collect())
...
topErrDate = sorted(errDateSorted.collect(), key=lambda x: x[1], reverse = True)[:5]
...
hourRecordsSum = hourCountPairTuple.countByKey()

hourRecordsSorted = sc.parallelize(hourRecordsSum.items()).cache()
...
hourRecordsSorted.map(lambda x: x[1])
...
hoursWithErrors404 = map(lambda x: x[0], hourRecordsSorted.collect())
errors404ByHours = map(lambda x: x[1], hourRecordsSorted.collect())
